...
angular
  .module('ion-autocomplete', [])
  .directive('ionAutocomplete', ionAutocomplete)

ionAutocomplete.$inject = ['$ionicBackdrop', '$ionicScrollDelegate', '$document', '$q', '$parse', '$interpolate', '$ionicPlatform', '$compile', '$templateRequest']
function ionAutocomplete ($ionicBackdrop, $ionicScrollDelegate, $document, $q, $parse, $interpolate, $ionicPlatform, $compile, $templateRequest) {
  return {
    ...
    controller: ['$attrs', '$timeout', '$scope', function ($attrs, $timeout, $scope) {
      ...
      var booleanValueOrDefault = function (value, defaultValue) {
        return !value ? defaultValue : (value.match(/^(true|yes|t|y|1)$/i))
      }

      var intValueOrDefault = function (value, defaultValue) {
      ...
      this.maxSelectedItems = intValueOrDefault($attrs.maxSelectedItems, undefined)
      ...
      this.manageExternally = booleanValueOrDefault($attrs.manageExternally, false)
      this.ngModelOptions = valueOrDefault($scope.$eval($attrs.ngModelOptions), {})
      this.closeOnSelect = booleanValueOrDefault($attrs.closeOnSelect, false)
      this.addOnEnter = booleanValueOrDefault($attrs.addOnEnter, false)
      this.removeOnBackspace = booleanValueOrDefault($attrs.removeOnBackspace, false)
    ...
    link: function (scope, element, attrs, controllers) {
      ...
      var template = [
        ...
        '<input type="search" id="\'ion-autocomplete-input-\'"' + ionAutocompleteController.randomCssClass + ' class="ion-autocomplete-search" ng-model="viewModel.searchQuery" ng-model-options="viewModel.ngModelOptions" placeholder="{{viewModel.placeholder}}"/>',
        '</label>',
        ...
        '<button class="ion-autocomplete-cancel button button-clear" on-tap="viewModel.cancelClick()">{{viewModel.cancelLabel}}</button>',
        '</div>',
        ...
        '<i class="icon ion-trash-a" style="cursor:pointer" on-tap="viewModel.removeItem($index)"></i>',
        '</ion-item>',
        ...
        '<i class="icon ion-trash-a" style="cursor:pointer" on-tap="viewModel.removeItem(0)"></i>',
        '</ion-item>',
        '<ion-item class="item-divider" ng-if="viewModel.searchItems.length > 0">{{viewModel.selectItemsLabel}}</ion-item>',
        '<ion-item ng-repeat="item in viewModel.searchItems" item-height="55px" item-width="100%" on-tap="viewModel.selectItem(item)" class="item-text-wrap">',
      ...
      $q.when().then(function () {
        ...
        // TODO: Fix this.
        // Use timeout as the template is added in the link function.
        ionAutocompleteController.timeout(function () {
          var innerInput

          if (ionAutocompleteController.addOnEnter) {
            if (!innerInput) {
              innerInput = angular.element('#ion-autocomplete-input-' + ionAutocompleteController.randomCssClass)
            }

            innerInput.bind('keydown keypress', function (event) {
              if (event.keyCode === 13) {
                if (ionAutocompleteController.searchItems[0]) {
                  ionAutocompleteController.selectItem(ionAutocompleteController.searchItems[0])
          ...
          if (ionAutocompleteController.removeOnBackspace) {
            if (!innerInput) {
              innerInput = angular.element('#ion-autocomplete-input-' + ionAutocompleteController.randomCssClass)
            }

            innerInput.bind('keydown keypress', (function () {
              var tagMarked = false

              return function (event) {
                if (event.keyCode === 8 && (typeof ionAutocompleteController.searchQuery === 'undefined' || ionAutocompleteController.searchQuery === '')) {

                  console.log("ctrl ", ionAutocompleteController, " tagMarked ", tagMarked)

                // TODO this doesn't look that good...
                  if (ionAutocompleteController.ngModel.length > 0) {
                    if (tagMarked) {
                      ionAutocompleteController.removeItem(ionAutocompleteController.selectedItems.length - 1)

                      tagMarked = false
                    } else {
                      var _item = ionAutocompleteController.selectedItems[ionAutocompleteController.selectedItems.length - 1]

                      if (_item !== null && typeof _item === 'object') {
                        _item.selectedForRemoval = true
                      }
        ...
        ionAutocompleteController.selectItem = function (item) {
          ...
          if (ionAutocompleteController.closeOnSelect) {
        ...
        ionAutocompleteController.removeItem = function (index) {
          ...
          if (!angular.isArray(ionAutocompleteController.selectedItems)) {
            ...
            ionAutocompleteController.ngModel = []
          } else {
            ...
            ionAutocompleteController.ngModel = ionAutocompleteController.ngModel.slice()
          }
        ...
        var isKeyValueInObjectArray = function (objectArray, key, value) {
          if (angular.isArray(objectArray)) {
            for (var i = 0, i_max = objectArray.length; i < i_max; i++) {
        ...
        if (!ionAutocompleteController.manageExternally) {
        ...
        ngModelController.$render = function () {
          element.val(ionAutocompleteController.modelRenderMethod(ngModelController.$viewValue))
        }
